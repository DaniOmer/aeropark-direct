    ...
    <div className="p-6 md:p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          ...
          <p className="text-xs text-muted-foreground mt-1">{occupancyPercent}% occupé</p>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          ...
          <p className="text-3xl font-extrabold text-foreground">
            {data.monthRevenue.toFixed(0)}<span className="text-lg"> €</span>
          </p>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border p-5">
          ...
          {data.pendingReservations > 0 && (
            <Link href="/admin/reservations" className="text-xs text-primary hover:underline mt-1 inline-block">
            ...
            <BarChart data={data.monthlyRevenue} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
              ...
              <Tooltip
                ...
                formatter={(value, name) => [
                  name === "revenue" ? `${value} €` : `${value} réservation${Number(value) > 1 ? "s" : ""}`,
                  name === "revenue" ? "Chiffre d'affaires" : "Réservations",
                ]}
                cursor={{ fill: "rgba(14, 165, 233, 0.08)" }}
              />
              <Bar yAxisId="left" dataKey="revenue" fill="#0ea5e9" radius={[4, 4, 0, 0]} name="revenue" />
              <Bar yAxisId="right" dataKey="reservations" fill="#f59e0b" radius={[4, 4, 0, 0]} name="reservations" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-2xl border border-border overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            ...
            <button
              onClick={() => downloadPDF(data.todayArrivals, "Arrivées du jour")}
          ...
          {data.todayArrivals.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">Aucune arrivée prévue</p>
            </div>
          ) : (
            <div className="divide-y divide-border max-h-80 overflow-y-auto">
              {data.todayArrivals.map((res) => (
                <div key={res.id} className="px-5 py-3 flex items-center justify-between hover:bg-secondary/30 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {res.vehicle_plate || "—"} · {formatVehicle(res.vehicle_type)}
        ...
        <div className="bg-card rounded-2xl border border-border overflow-hidden">
          <div className="px-5 py-4 border-b border-border flex items-center justify-between">
            ...
            <button
              onClick={() => downloadPDF(data.todayDepartures, "Départs du jour")}
              className="flex items-center gap-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors px-2 py-1 rounded-lg hover:bg-secondary"
              title="Télécharger PDF"
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {data.todayDepartures.length === 0 ? (
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">Aucun départ prévu</p>
            </div>
          ) : (
            <div className="divide-y divide-border max-h-80 overflow-y-auto">
